...
contract LazyCoin is ERC20, ERC20Permit, ReentrancyGuard, Ownable, IWormholeReceiver {
    ...
    struct User {
        ...
        uint256 firstCheckTs;
        uint256 lastTotalRewards;
    ...
    event RewardsUpdated(uint256 newRewardPoolBalance, uint256 totalRewards, uint256 availableRewards);
    /// @notice Emitted when a reward is issued to a user.
    event RewardIssued(address indexed recipient, uint256 amount, uint256 timestamp);
    /// @notice Emitted when the marketing wallet address is updated.
    ...
    constructor(
        ...
        users[_recipient].isExcludedFromReward = true;
        users[marketingWalletAddress].isExcludedFromFee = true;
        users[marketingWalletAddress].isExcludedFromReward = true;
        users[rewardPoolAddress].isExcludedFromFee = true;
        users[rewardPoolAddress].isExcludedFromReward = true;
        users[multiChainWalletAddress].isExcludedFromFee = true;
        users[multiChainWalletAddress].isExcludedFromReward = true;
        users[address(this)].isExcludedFromFee = true;
        users[address(this)].isExcludedFromReward = true;
        users[uniswapPair].isExcludedFromFee = true;
    ...
    /**
     * @notice Updates token balances during transfers, minting, and burning while applying fees and processing reward calculations.
     * @dev Overrides the internal token transfer hook.
     *      - For minting (when `from == address(0)`), increases totalSupply_ by the full amount (no fees applied).
     *      - For transfers, calculates fees using `calculateFees`, processes fees with `_handleFees`, and updates reward data
     *        (via `calcReward`) before deducting the full value from the sender's balance.
     *      - For burning (when `to == address(0)`), decreases totalSupply_ by the full amount.
     *      - Credits the recipient with the fee-adjusted amount.
     *      Emits a single {Transfer} event for the net transferred amount; additional Transfer events may be emitted for fee distributions.
     *
     * @param from The address initiating the transfer (or minting if zero).
     ...
     *         - For transfers, the sender’s balance decreases by value and the recipient’s balance increases by the fee-adjusted amount.
     *         - Fees are distributed and reward calculations are updated as applicable.
     */
    function _update(address from, address to, uint256 value) internal override {
        ...
        if (from == address(0)) {
            ...
            initializeUserReward(from);
            // Calculate fees and obtain the fee-adjusted transfer value.
            (
                whaleFee, 
                marketingFee, 
                rewardFee, 
        ...
        if (whaleFee > 0) emit Transfer(from, rewardPoolAddress, whaleFee);
        if (marketingFee > 0) emit Transfer(from, marketingWalletAddress, marketingFee);
        if (rewardFee > 0) emit Transfer(from, rewardPoolAddress, rewardFee);
        if (liquidityFee > 0) emit Transfer(from, address(this), liquidityFee);
    }

    /**
     * @notice Returns the total number of tokens in existence.
     * @dev Overrides ERC20.totalSupply to return the custom totalSupply_ value.
     ...
     * @dev Overrides ERC20.balanceOf to retrieve the balance from the custom users mapping.
     ...
     *
     * @return whaleFee The computed whale fee.
     * @return marketingFee The computed marketing fee.
     * @return rewardFee The computed reward fee.
     * @return liquidityFee The computed liquidity fee.
     * @return finalAmount The net transfer amount after deducting all fees.
     */
     ...
     * @param rewardFee The reward fee amount.
     * @param liquidityFee The liquidity fee amount.
     */
    function _handleFees(
        ...
        if (rewardFee > 0) collectRewardFee(rewardFee);
        if (marketingFee > 0) collectMarketingFee(marketingFee);
        if (whaleFee > 0) collectRewardFee(whaleFee);
        if (liquidityFee > 0) collectLiquidityFee(liquidityFee);
    }
    ...
    function collectMarketingFee(uint256 feeAmount) internal {
        users[marketingWalletAddress].balance += feeAmount;
    }
    ...
    function collectLiquidityFee(uint256 feeAmount) internal {
        users[address(this)].balance += feeAmount;
    }
     ...
     *      6. Emits a {RewardIssued} event and a Transfer event from rewardPoolAddress to the caller.
     *
    ...
    function getRewardRelevantSupply() internal view returns (uint256) {
        uint256 relevantSupply = totalSupply_;
        relevantSupply -= users[rewardPoolAddress].balance;
        relevantSupply -= users[marketingWalletAddress].balance;
        relevantSupply -= users[multiChainWalletAddress].balance;
        return relevantSupply;
    }
    ...
    /**
     * @notice Swaps a portion of the contract's tokens for ETH and adds liquidity to Uniswap.
     * @dev Splits the provided token balance into two halves, swaps one half for ETH, and then pairs it with the other half
     *      to add liquidity via Uniswap. Resets the contract's token balance (users[address(this)].balance) to zero.
     *
     * @param contractTokenBalance The token balance available for swap-and-liquify.
     */
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 wethBalance = address(this).balance - initialBalance;
    ...
    /**
     * @notice Swaps a specified amount of tokens for ETH using Uniswap V2.
     * @dev Approves the Uniswap router to spend tokens, sets up the token-to-WETH swap path, and executes the swap.
     *
    ...
    /**
     * @notice Adds liquidity to Uniswap V2 by pairing tokens with ETH.
     * @dev Approves the Uniswap router to spend tokens and calls addLiquidityETH.
     *
    ...
    /**
     * @notice Fallback function to accept incoming ETH.
     */
